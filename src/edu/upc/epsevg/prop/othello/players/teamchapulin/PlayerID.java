...
/**
 * Jugador MiniMaxID que implementa el mecanisme de timeout.
 * 
...
public class PlayerID implements IPlayer, IAuto {
    ...
    /**
     * Constructor del jugador IA MiniMaxID amb timeout.
     * 
     * Es dona per defecte el nom del jugador i es crea la taula de transposicions i 
     * el vector tridimensional de les claus de Zobrist.
     * 
     */
    public PlayerID() {
        this.nom = "MiniMaxID";
    ...
    */
    ...
    public Move move(GameStatus s) {
        ...
        nodesExplorats = 0;depth = 0;int valor = 0;
        ...
        if(s.currentPlayerCanMove()){
            long hashKey;
            //Mentre no s'acabi el temps, continua
            while(temps){
                valor = Integer.MIN_VALUE;
                depth++;
                //Ordenem els moviments posant els millors primer
                moves = ordena(moves, millorMov);
                ...
                for(int i = 0; i < moves.size(); ++i){
                    ...
                    if(gs_nou.checkGameOver()){
                        return new Move(moves.get(i), nodesExplorats, depth, SearchType.MINIMAX_IDS);
                    }
                    ...
                    int candidat = miniMax(gs_nou,depth,depth,Integer.MIN_VALUE,Integer.MAX_VALUE,false, hashKey);
                    ...
                    //Si s'ha acabat el temps acaba el bucle
                    if(!temps) break;
                }  
            }
        }   else{s.skipTurn();} //El jugador no pot tirar, pasa torn
        
        return new Move(moves.get(millorMov), nodesExplorats, depth, SearchType.MINIMAX_IDS);
    }
    ...
    /**
     * Ordena els moviments del node.
     * 
     * Intercanvia els moviments, posant els millors moviments dels primers de la llista.
     * 
     * @param moves llista de moviments
     * @param millorMov index del millor moviment a intercanviar
     * @return una llista nova, amb el millor moviment rebut el primer.
     */
    private ArrayList<Point> ordena(ArrayList<Point> moves, int millorMov) {
        Collections.swap(moves, 0, millorMov);
        return moves;
    }
